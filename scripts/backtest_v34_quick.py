#!/usr/bin/env python3
"""V3.4 Stage 1 Behavioral Backtest."""
...
ckpt = torch.load("/home/user/RabitPropfim/models_saved/best_v34_stage1.pt", map_location=device, weights_only=False)
m = PPOActorCritic().to(device)
m.load_state_dict(ckpt["model_state_dict"])
m.eval()
print(f"V3.4 Stage 1 loaded (step={ckpt.get('step', 0)})")
...
for sym in syms:
    ...
    print(f"{sym:12s} | Trades={tot:5d} | WR={wr:5.1f}% | ManualClose={mc:4d} (Win={mcw}) | SL={sl:4d} | AvgDur={avg_dur:.1f} | "
...
print(f"OVERALL: {total_all_t} trades | WR={total_wr:.1f}% | ManualClose={total_manual} (WR={mc_wr:.1f}%) | SL_Hit={total_sl}")
print("=" * 100)

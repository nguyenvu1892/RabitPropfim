...
ckpt = torch.load("/home/user/RabitPropfim/models_saved/best_v33_stage2.pt", map_location=device, weights_only=False)
m = PPOActorCritic().to(device)
m.load_state_dict(ckpt["model_state_dict"])
m.eval()
print(f"Stage 2 model loaded (step={ckpt.get('step', 0)})")

#!/usr/bin/env python3
"""
V3.4 PPO Training -- "Quan Tri Rui Ro"

Stage 1: Discrete(4) BUY/SELL/HOLD/CLOSE
  - 3-TF obs: M15(50) + M5(50) + M1×5(250) = 350-dim
  - Auto SL from M5 swing points
  - Reward: PnL + CLOSE profit ×5
  - No frequency pressure

Usage:
    python scripts/train_v34.py --stage 1 --n-envs 12
    python scripts/train_v34.py --stage 1 --test
"""
...
logger = logging.getLogger("v34_train")

SYMBOLS = ["XAUUSD", "BTCUSD", "ETHUSD", "US30_cash", "US100_cash"]


# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# PPO MODEL -- V3.4 (350-dim obs, 4 actions)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

class PPOActorCritic(nn.Module):
    def __init__(self, obs_dim: int = 350, n_actions: int = 4, hidden_dims: list | None = None):
        super().__init__()
...
def train_stage1(
    ...
    logger.info("=" * 70)
    logger.info("  V3.4 STAGE 1 -- PPO Discrete(4) BUY/SELL/HOLD/CLOSE")
    ...
    best_checkpoint = MODELS_DIR / "best_v34_stage1.pt"
    ckpt_dir = MODELS_DIR / "v34_stage1"
    ...
    elapsed = time.time() - start_time
    logger.info("=" * 70)
    logger.info("  V3.4 STAGE 1 COMPLETE!")
...
def train_stage2(
    ...
    logger.info("=" * 70)
    logger.info("  V3.4 STAGE 2 -- PPO + IL (Self-Imitation Learning)")
    ...
    model = PPOActorCritic(obs_dim=obs_dim, n_actions=4).to(device)
    stage1_path = MODELS_DIR / "best_v34_stage1.pt"
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, eps=1e-5)
    buffer = RolloutBuffer(n_steps, n_envs, obs_dim, device)
    best_checkpoint = MODELS_DIR / "best_v34_stage2.pt"
    ...
    elapsed = time.time() - start_time
    logger.info("=" * 70)
    logger.info("  V3.4 STAGE 2 COMPLETE!")
    logger.info("  Steps: %d in %.1fs (%.0f SPS)", global_step, elapsed, global_step / max(elapsed, 1))
    logger.info("  BUY=%.1f%% SELL=%.1f%% HOLD=%.1f%% CLOSE=%.1f%%", pct_buy, pct_sell, pct_hold, pct_close)
    logger.info("  Checkpoint: %s", best_checkpoint)
    logger.info("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="V3.4 PPO Training")
    ...
    if args.stage == 1:
        ...
        logger.error("V3.4 supports --stage 1 or 2")
        sys.exit(1)
